...
export async function getBySlugHandler(req: Request, res: Response, next: NextFunction) {
  try { const store = await service.getStoreBySlug(req.params.slug); if (!store) return res.status(404).json({ error: 'Boutique non trouvée' }); res.json(store); } catch (e) { next(e); }
}
