-- GawJaay V2 — 000_base : schéma V1 (portage PostgreSQL fidèle de backend/src/lib/db.ts)
-- Généré mécaniquement puis vérifié par la comparaison de parité (validate.mjs).
-- Règle : aucune transformation manuelle du schéma ; dialecte uniquement
--   * datetime('now') -> now()::text (colonnes TEXT, l'app écrit des ISO strings)
--   * SQLite PRAGMA n/a en PG
